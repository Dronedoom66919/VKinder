...
from vk_functions import (
    ...
    sort_likes)
from models import (
    create_db, write_msg, register_user, add_user,  
...
def go_to_favorites(ids):
    ...
    for nums, users in enumerate(alls_users):
        res = users_get(users[0])
        write_msg(
            ids, 
            f"""{res["first_name"]} {res["last_name"]},
            https://vk.com/id{res["id"]}"""
        )
